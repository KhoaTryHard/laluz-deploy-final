...
export async function POST(request) {
  try {
    ...
    const { 
      ...
      brand_id, is_new_brand, new_brand_name,
      volume_ml, image_urls, top_notes, middle_notes, base_notes 
    } = body;

    // Xử lý Brand
    const finalBrandId = await getOrCreateBrandId(is_new_brand, brand_id, new_brand_name);
    ...
    const productResult = await query({
      query: `
        ...
        VALUES (?, ?, ?, ?, ?, ?, ?)
      `,
      values: [name, slug, price, stock_quantity, description, finalBrandId, volume_ml], 
    });
